...
An autonomous language serves as abstraction above writing actual commands in code. Writing your autonomous in code tends to be quite slow, since you have to re-deploy every time you change anything. 

Autonomous language helps you write strings of words, that will then be parsed in to commands on the robot. This removes the need for re-deployment, as they can be passed into the code from the dashboard of your choice, and allows you to change autonomous on the fly (pretty much up until the start of autonomous). Being adaptive with autonomous was an important ability for a team to have during the 2018 season, due to the dynamic nature of the game.

## How could an autonomous language look like?

**Any way you want it to look!** 

Let me show you an example of a syntax that you might use, since it's one that's easy to implement. This is how our command syntax for the 2018 season looked like:

```
Function_1(parameter_1, parameter_2,... parameter_n)
```

This is what a real sequence of commands on your robot could look like: `Drive(30.5) TurnBy(50) Outtake(0.5)`. The robot should drive 30.5 inches, turn by 50 degrees, and outtake for 0.5 seconds.
...
This doesn't work well. Not only does it leave the whitespace around the parameters (for commands with more than 1 parameter), it doesn't even split the first parameter from the name of the command! 
...
* **Optional parameters** - sometimes, you want to adjust a variable of the command that would otherwise be left to its default value (speed of driving, sequential / parallel). You can adjust the parser to take this into account. An example would be `Drive(50, Speed=0.3, Mode=Parallel)`.
* **Inverting commands** - when mirorring a series of commands, manually changing the values of all TurnBy commands would be a pain. Instead, an example of what you can do (using the code that we developed in this guide) is to pass a `boolean reverse` into the constructor of the AutonomousCommandGroup class. Then, inside our switch for the case `TurnBy`, if `reverse` is `true`, multiply the values of the variable `angle` in our switch by `-1` (from left to right and vice versa).
* **Recursively parsing commands** - a neat concept is to store your basic command Strings in an array and create "higher commands" by combining them. You can create a new command called `C(n)` that replaces itself by the basic command (by looking at the nth index of the basic commands array. You can even create another command that uses that command (just make sure that if you use a `C(0)` command, don't make the zero-th index of the basic commands array `C(0)` - that would cause a stack overflow!).
